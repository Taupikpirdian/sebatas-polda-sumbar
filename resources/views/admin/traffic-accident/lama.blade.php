  ...
  <div class="row page-titles mx-0">
    <div class="col-sm-6 p-md-0">
      <div class="welcome-text">
        <h4>Data Kecelakaan Lalu Lintas Lebih Dari {{ $month }} bulan</h4>
      </div>
    </div>
    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"><a href="javascript:void(0)">Data Kecelakaan Lalu Lintas Lama</a>
        </li>
      </ol>
    </div>
  </div>
    ...
    <div class="col-lg-12">
      <div class="card">
        <div class="card-header">
          {!! Form::open(['method'=>'GET','url'=>'/laka-lantas-lama','role'=>'search'])  !!}
            <div class="input-group" style="float: left; width: 300px; margin-right: 10px">
              <select class="form-control" name="month" required>
                  <option value="">Pilih Usia Perkara</option>
                  <option value="3" @if($month == '3') selected @endif>> 3 bulan</option>
                  <option value="6" @if($month == '6') selected @endif>> 6 bulan</option>
                  <option value="12" @if($month == '12') selected @endif>> 1 tahun</option>
              </select>
            </div>
            <div class="input-group" style="float: left; width: 300px; margin-right: 10px">
              <input type="text" class="form-control float-right" name="search" value="{{ $search_bar }}" placeholder="Search no LP, korban dan tersangka ...">
            </div>
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table class="table table-responsive-md">
              ...
              <tbody>
                @foreach($traffict as $i=>$traffic_accident)
                <tr>
                    <td>{{ ($traffict->currentpage()-1) * $traffict->perpage() + $i + 1 }}</td>
        ...
        <div class="card-footer clearfix">
          <ul class="float-left">
            Showing {{ (($traffict->currentpage()-1) * $traffict->perPage())+1 }} 
            to {{ $traffict->currentPage()*$traffict->perPage() }} 
            of {{ $traffict->total() }} entries
          </ul>
